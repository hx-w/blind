...
async fn share(
    ...
    let payload=api(&c.server,"/api/v1/client/scenes",&c.credential,Some(json!({"paths":if input.manifest.is_some(){Vec::<String>::new()}else{paths},"manifest":input.manifest,"title":input.title,"labels":input.labels.meshes,"label_groups":input.labels.groups,"origin":host,"stateless":stateless}))).await?;
